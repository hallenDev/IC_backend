...
use crate::{mutate_state, RuntimeState, LOCAL_USER_INDEX_CANISTER_INITIAL_CYCLES_BALANCE};
use canister_api_macros::proposal;
use local_user_index_canister::init::Args as InitLocalUserIndexCanisterArgs;
use post_index_canister::{Event as PostIndexEvent, LocalUserIndexAdded};
...
struct PrepareOk {
    ...
    init_canister_args: InitLocalUserIndexCanisterArgs,
}


fn prepare(args: &Args, state: &mut RuntimeState) -> Result<PrepareOk, Response> {
    let cycles_to_use = if args.canister_id.is_none() {
        let cycles_required = LOCAL_USER_INDEX_CANISTER_INITIAL_CYCLES_BALANCE + CREATE_CANISTER_CYCLES_FEE;
    ...
    let canister_wasm = state.data.local_user_index_canister_wasm_for_new_canisters.clone();
    let init_canister_args = InitLocalUserIndexCanisterArgs {
        user_index_canister_id: state.env.canister_id(),
        post_index_canister_id: state.data.post_index_canister_id,
        local_post_index_canister_ids: state.data.local_post_index_canister_ids.clone(),
...
fn commit(canister_id: CanisterId, wasm_version: Version, state: &mut RuntimeState) {
    ...
    state.push_event_to_post_index(PostIndexEvent::LocalUserIndexAdded(Box::new(LocalUserIndexAdded{
        canister_id,
    })));
}